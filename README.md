...
| Adaptive UF-SSL | Confidence + linearly-decaying entropy threshold |
| UF-SSL + Weighted Loss | Stage 2: entropy-weighted sample loss |
| Cost-Sensitive Asymmetric | Stage 6: asymmetric class penalty + asymmetric gate |
| FNR-Driven Defer Gate | Stage 7: FNR feedback loop with adaptive benign gate |

## Project structure

```
.
├── run_experiments.py   # End-to-end runner — produces results table + plots
├── requirements.txt
└── src/
...
# Plots saved to ~/Desktop by default; override with --save_dir /path/to/dir
```
